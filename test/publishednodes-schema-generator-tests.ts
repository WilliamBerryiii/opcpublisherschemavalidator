...
describe('An example publishednodes.json file', () => {
  ...
  it('should not return an error when validating a mixed-mode schema', () => {
  ...
  it('should not return an error on a highly customized and multi-version capable pn.json file', () => {
  ...
  it('should not return an error when being validated with requireUseSecurity set to true, and when the UseSecurity flag is false', () => {
    ...
    const pn = JSON.parse(JSON.stringify(publishedNodes));

    const [result, validationErrors] = validateFile(pn, requireSecuritySchema);
    //console.log(validationErrors);
